...
#import "AW_ChatTimeCell.h"//时间cell
...
@interface ChatViewController ()<UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SRRefreshDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate, LocationViewDelegate, EMCDDeviceManagerDelegate,ChatViewControllerDelegate>
{
...
/**
 *  @author cao, 15-11-19 16:11:15
 *
 *  图片数组
 */
@property(nonatomic,strong)NSMutableArray * photosArray;
/**
 *  @author cao, 15-11-19 16:11:44
 *
 *  总的用户信息
 */
...
-(NSMutableDictionary*)totalDict{
    if (!_totalDict) {
        ...
        UserDataBaseHelper * dataHelper = [[UserDataBaseHelper alloc]init];
        //查看所有数据
       _totalDict = [dataHelper queryAllUserInfo];
    }
    return _totalDict;
}
...
{
    UIBarButtonItem * leftBtn = [[UIBarButtonItem alloc]initWithImage:[[UIImage imageNamed:@"返回"]imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] style:UIBarButtonItemStylePlain target:self action:@selector(back)];
    self.navigationItem.leftBarButtonItem = leftBtn;
    
    UIBarButtonItem * rightButton = [[UIBarButtonItem alloc]initWithImage:[[UIImage imageNamed:@"主页"]imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] style:UIBarButtonItemStylePlain target:self action:@selector(clickedMainPage)];
    self.navigationItem.rightBarButtonItem = rightButton;
}

-(void)clickedMainPage{
    AW_MyDetailViewController * detailController = [[AW_MyDetailViewController alloc]init];
    detailController.hidesBottomBarWhenPushed = YES;
    self.navigationController.navigationBar.hidden = NO;
    detailController.artStoreBtnTag = 0;
    detailController.person_id =  self.shoper_id;
    detailController.shop_id = self.shop_id;
    detailController.shop_state = @"3";
    detailController.productionDataSource.person_id = self.shoper_id;
    detailController.attentionDataSource.person_id = self.shoper_id;
    detailController.fansDataSource.person_id = self.shoper_id;
    detailController.dynamicDataSource.person_id = self.shoper_id;
...
- (SRRefreshView *)slimeView
{
    if (_slimeView == nil) {
        ...
        _slimeView.slime.skinColor = HexRGB(0x88c244);
...
- (UITableView *)tableView{
    if (_tableView == nil) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0,0, kSCREEN_WIDTH, kSCREEN_HEIGHT - self.chatToolBar.frame.size.height + 20) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            AW_ChatTimeCell * timeCell = [tableView dequeueReusableCellWithIdentifier:cellId];
            if (!timeCell) {
                timeCell = BundleToObj(@"AW_ChatTimeCell");
                timeCell.backgroundColor = [UIColor clearColor];
                timeCell.contentView.backgroundColor = [UIColor clearColor];
                timeCell.selectionStyle = UITableViewCellSelectionStyleNone;
            }
            
            timeCell.sendTime.text = (NSString *)obj;
...
- (id <MWPhoto>)photoBrowser:(MWPhotoBrowser *)photoBrowser photoAtIndex:(NSUInteger)index {
    if (index < self.photosArray.count)
        return [self.photosArray objectAtIndex:0];
    return nil;
}
...
//请求信息存到数据库
-(void)requestphoneString:(NSString*)string userid:(NSString*)user_id{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSDictionary * userDict = @{@"param":@"nickNameAndHead",@"jsonParam":str};
        AFHTTPSessionManager * manager = [AFHTTPSessionManager manager];
        [manager POST:ARTSCOME_INT parameters:userDict success:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull responseObject) {
            NSLog(@"%@",responseObject);
            NSArray * userArray = responseObject[@"info"];
            if ([responseObject[@"code"]intValue] == 0) {
                [userArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    NSDictionary * infoDict = obj;
                    AW_EMUserModal * modal = [[AW_EMUserModal alloc]init];
                    modal.user_id = infoDict[@"id"];
                    modal.headImage = infoDict[@"head_img"];
                    modal.IM_phone = infoDict[@"phone"];
                    modal.nickName = infoDict[@"nickname"];
                    //插入数据库
                    if (![[self.totalDict allKeys] containsObject:infoDict[@"phone"]]) {
                        [dataHelper addCustomer:modal];
                        
                        [self.totalDict setValue:infoDict[@"head_img"] forKey:infoDict[@"phone"]];
                    }
...
- (NSArray *)formatMessages:(NSArray *)messagesArray
{
    ...
    NSLog(@"%@",user_phone);
    NSLog(@"%@",self.shopIM_phone);
    NSString * phoneString;
   
    //首先查看数据库中是否已经有头像
    UserDataBaseHelper * helper = [[UserDataBaseHelper alloc]init];
    NSDictionary * tmpDict = [helper queryAllUserInfo];
    if ([tmpDict valueForKey:user_phone]&&[tmpDict valueForKey:self.shopIM_phone]){
       //都存在的情况从数据库中取
        
    }else{
    phoneString = [[user_phone stringByAppendingString:@","]stringByAppendingString:self.shopIM_phone];
        [self requestphoneString:phoneString userid:user_id];
  }
    ...
    if ([messagesArray count] > 0) {
        for (EMMessage *message in messagesArray) {
            ...
            if ([_delelgate respondsToSelector:@selector(avatarWithChatter:)]) {
                ...
                NSLog(@"%@",model.headImageURL);
            }else{
                model.headImageURL = [self.totalDict valueForKey:model.nickName];
            }
...
-(NSMutableArray *)formatMessage:(EMMessage *)message
{
    ...
    if ([_delelgate respondsToSelector:@selector(avatarWithChatter:)]) {
        ...
        model.headImageURL = [self.totalDict valueForKey:model.username];
    }
