...
#import "AW_OrderWaitPayDataSource.h"
...
@implementation AW_OrderWaitPayDataSource
...
-(AW_OrderStateCell*)configOrderStateCellWithModal:(AW_MyOrderModal *)modal tableView:(UITableView *)tableView indexPath:(NSIndexPath *)indexPath {
    ...
    cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    if (!cell) {
        cell = [[NSBundle mainBundle]loadNibNamed:@"AW_OrderStateCell" owner:self options:nil][0];
    }
    ...
    cell.didClickedWaitPayCellBtns = ^(NSInteger index){
        ...
        if (_didClickedPayCellButtons) {
            _didClickedPayCellButtons(_orderModal,index);
        }
    };
    return cell;
}
