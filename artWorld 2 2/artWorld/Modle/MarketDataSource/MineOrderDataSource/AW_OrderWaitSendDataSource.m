...
#import "AW_OrderWaitSendDataSource.h"
...
@implementation AW_OrderWaitSendDataSource
-(void)getTextData{
...
-(AW_OrderStateCell*)configOrderStateCellWithModal:(AW_MyOrderModal *)modal tableView:(UITableView *)tableView indexPath:(NSIndexPath *)indexPath{
    ...
    cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    if (!cell) {
        cell = [[NSBundle mainBundle]loadNibNamed:@"AW_OrderStateCell" owner:self options:nil][1];
    }
    ...
    cell.didClickRemindBtn = ^(){
        UIView * v = [weakCell.remindSnedBtn superview];
        UIView * v1 = [v superview];
        AW_OrderStateCell * tmpCell = (AW_OrderStateCell*)[v1 superview];
        NSIndexPath * path = [tableView indexPathForCell:tmpCell];
        AW_MyOrderModal * tmpModal = self.dataArr[path.row];
        _orderModal = tmpModal;
        if (_didClickedRemindButton) {
            _didClickedRemindButton(_orderModal);
        }
    };
    return cell;
}
