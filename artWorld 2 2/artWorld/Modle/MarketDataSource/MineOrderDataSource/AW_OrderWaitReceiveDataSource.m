//
//  AW_OrderWaitReceiveDataSource.m
//  artWorld
//
...
#import "AW_OrderWaitReceiveDataSource.h"
...
@implementation AW_OrderWaitReceiveDataSource
-(void)getTextData{
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    AW_MyOrderModal * orderModal = self.dataArr[indexPath.row];
...
-(AW_OrderStateCell*)configOrderStateCellWithModal:(AW_MyOrderModal *)modal tableView:(UITableView *)tableView indexPath:(NSIndexPath *)indexPath {
    ...
    cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    if (!cell) {
        cell = [[NSBundle mainBundle]loadNibNamed:@"AW_OrderStateCell" owner:self options:nil][2];
    }
    ...
    cell.didClickedWaitReceiveCellBtns = ^(NSInteger index){
        ...
        if (_didClickedReceiveCellBtns){
            _didClickedReceiveCellBtns(_orderModal,index);
        }
    };
    return cell;
}
