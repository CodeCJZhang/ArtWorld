//
//  AW_KeyWordSearchController.m
//  artWorld
//
//  Created by 曹学亮 on 15/10/28.
//  Copyright © 2015年 张晓旭. All rights reserved.//

#import "AW_KeyWordSearchController.h"
#import "AW_keywordSearchView.h"
#import "AW_Constants.h"
#import "AW_SearchStringCell.h"//搜索关键字cell
#import "AW_SearchReaultController.h"//搜索结果列表
#import "MBProgressHUD.h"
#import "ArtSearchDataBaseHelper.h"//存到本地数据库

@interface AW_KeyWordSearchController ()<UITableViewDataSource,UITableViewDelegate>
/**
...
@implementation AW_KeyWordSearchController

-(NSMutableArray*)searchArray{
    if (!_searchArray){
        NSMutableArray * tmpArray = [[[ArtSearchDataBaseHelper alloc]init]queryAllKeyWord];
        if (tmpArray.count > 0) {
    ...
    self.searchView.didClickedSearchBtn = ^(NSString * searchString){
        ...
        if (searchString.length == 0) {
            ...
            if (![weakSelf.searchArray containsObject:searchString]) {
                ...
                //将艺术品搜索关键字存到数据库
                ArtSearchDataBaseHelper * helper = [[ArtSearchDataBaseHelper alloc]init];
                NSMutableArray * tmpArray = [helper queryAllKeyWord];
                if (![tmpArray containsObject:searchString]) {
                    [helper addKeyWord:searchString];
                }
            }
            //跳转到艺术品搜索列表
            AW_SearchReaultController * resultController = [[AW_SearchReaultController alloc]init];
            resultController.hidesBottomBarWhenPushed = YES;
            resultController.searchDataSource.keyString = searchString;
